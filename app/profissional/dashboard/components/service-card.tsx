...
function CategoryBadge({ category }: { category: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold tracking-wider uppercase ${colors.bg} ${colors.text}`}
    >
...
export function ServiceCard({
      ...
      >
        ...
        <div className="relative h-48 w-full overflow-hidden bg-gray-100">
          ...
          <div className="absolute top-3 left-3 flex items-center gap-2">
            {isNew && (
              <span className="rounded-full bg-yellow-400 px-2.5 py-1 text-[10px] font-extrabold tracking-wider text-yellow-900 uppercase shadow-sm">
                Novo
              </span>
            )}
            {highlighted && (
              <span className="rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-extrabold tracking-wider text-gray-800 uppercase shadow-sm backdrop-blur-sm">
                ✦ Destaque
              </span>
            )}
          </div>

          {/* TODO: analisar se será utilizado o botão de salvar serviço */}
          {/* Save button
          <button
        ...
        <div className="flex flex-1 flex-col gap-3 p-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full border border-gray-200 bg-gray-50 px-2 py-0.5 text-[10px] font-semibold tracking-wider text-gray-500 uppercase">
              {type}
            </span>
          </div>

          {/* Title */}
          <h3 className="line-clamp-2 text-base leading-snug font-bold text-gray-900 transition-colors group-hover:text-gray-700">
            {title}
          </h3>
  ...
  return (
    <div
      className={`group flex flex-col gap-4 overflow-hidden rounded-2xl border bg-white transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg sm:flex-row ${
        highlighted
          ? "border-yellow-300 shadow-md ring-1 ring-yellow-200"
          : "border-gray-200 shadow-sm hover:border-gray-300"
      }`}
    >
      ...
      <div className="flex flex-1 flex-col gap-3 p-4 sm:py-5 sm:pr-5">
        ...
        {/* Title */}
        <h3 className="text-lg leading-snug font-bold text-gray-900 transition-colors group-hover:text-gray-700">
          {title}
        </h3>
