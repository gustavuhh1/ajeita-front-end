...
export const ProviderCard = ({ provider, viewMode = 'grid' }: ProviderCardProps) => {
  ...
  return (
    <div className={`
      ...
      ${isGrid ? 'rounded-[32px] p-6 flex flex-col' : 'rounded-2xl p-4 flex items-center gap-6'}
    `}>
      
      {/* Lado Esquerdo / Topo: Avatar e Favorito */}
      <div className={`flex ${isGrid ? 'justify-between items-start mb-4' : 'items-center'}`}>
      ...
      {/* Centro: Informações */}
      <div className={`grow ${isGrid ? 'space-y-4' : 'grid grid-cols-2 gap-4 items-center'}`}>
        <div>
      ...
      <div className={`
        ${isGrid ? 'pt-4 border-t border-gray-50 flex items-center justify-between mt-4' : 'flex items-center gap-6 shrink-0'}
      `}>
        ...
        <button className={`
          bg-yellow-400 hover:bg-yellow-500 text-gray-900 font-bold transition-colors shadow-sm flex items-center gap-2
          ${isGrid ? 'px-6 py-2.5 rounded-xl text-sm' : 'px-5 py-3 rounded-2xl text-base'}
        `}>
